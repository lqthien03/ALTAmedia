...
<div class="col-10 device">
    ...
    <h2>Quản lý tài khoản</h2>
        <form action="{{route('setting.update_account',$user)}}" method="post">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="formGroupExampleInput" placeholder="" name="name" value="{{old('name') ?? $user->name}}">
                        </div>

                        <div class="mb-3">
                            <label for="formGroupExampleInput" class="form-label">Số điện thoại : *</label>
                            <input type="text" class="form-control" id="formGroupExampleInput" placeholder="" name="sdt" value="{{old('sdt') ?? $user->sdt}}">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput" class="form-label">Email : *</label>
                            <input type="text" class="form-control" id="formGroupExampleInput" placeholder="" name="email" value="{{old('email') ?? $user->email}}">
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select form-control" aria-label="Default select example" name="role_id">
                                ...
                                @foreach ($role as $op )
                                        ...
                                        <option {{$user->role_id == $op->id ? 'selected' : ''}} value="{{$op->id}}">{{$op->name_role}}</option>
                                    @endforeach
                            </select>
                    ...
                    <div class="col-6 mt-">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="formGroupExampleInput" placeholder="" name="name_dangnhap" value="{{old('name_dangnhap') ?? $user->name_dangnhap}}">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput" class="form-label">Mật khẩu : *</label>
                            <input type="password" class="form-control" id="formGroupExampleInput" placeholder="" required name="password" value="{{old('password') ?? $user->password}}">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput" class="form-label">Nhập lại mật khẩu : *</label>
                            <input type="password" class="form-control" id="formGroupExampleInput" placeholder="" password name="password" value="{{old('password') ?? $user->password}}">
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select form-control" aria-label="Default select example" name="id_status_activate">
                                <option>Chọn tình trạng</option>
                                    @foreach ($status as $op )
                                        ...
                                        <option {{$user->id_status_activate == $op->id ? 'selected' : ''}} value="{{$op->id}}">{{$op->name_activate}}</option>
                                    @endforeach
                            </select>
                        </div>
